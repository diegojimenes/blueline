...
[Marcos do Projeto](#-marcos-implementados-m0m13)
...
[ Nível 3: Classe ]  ──> Métodos e Membros da classe & Grafo de Chamadas
        │
[ Nível 4: Método ]  ──> Inspeção de Código-fonte e Assinatura
        │
[ Nível 5: Local ]   ──> Funções aninhadas / closures e fluxo interno
```
...
- 🌐 **Domínio (`domain`)**: Agrupa símbolos por contextos de negócio configurados no `codeatlas.json`.
- 🔗 **Acoplamento (`coupling`)**: Destaca nós centrais e calcula métricas de acoplamento e dependência.
...
Ao inspecionar uma classe ou método no Nível 3, chamadas a entidades externas são renderizadas como **Portais Determinísticos** nas laterais do Canvas (entradas à esquerda, saídas à direita), permitindo saltar diretamente para módulos externos seguindo o fluxo de execução.

### 🔎 6. Busca Fuzzy Global Instantânea (`Ctrl+P` / `Cmd+P` / `/`)
Modal `QuickSearch` com busca difusa de altíssima performance para encontrar instantaneamente qualquer módulo, classe, interface ou método por nome ou caminho canônico.

### 📊 7. Diff Estrutural & Snapshots de Grafo
- Visualizador de **Git Diff unificado** com syntax highlighting para adições, deleções e hunks.
...
### 🤖 8. Protocolo de Agente & Contexto Pronto para LLMs
- Extração determinística de contexto de símbolos, métodos e referências cruzadas formatadas para injeção em prompts de IA.
- Indicador de atenção de IA na `StatusBar` e na barra de revisão com destaque dos nós em foco pelo agente.
...
### 🚀 10. Performance O(1) com Spatial Grid Hash & Cache Persistente
- **Spatial Grid Index**: Culling espacial com tempo de resposta $O(1)$, garantindo 60 FPS estáveis mesmo em repositórios com dezenas de milhares de nós.
- **Graph Cache Storage**: Armazenamento e restauração de snapshots em cache para boot instantâneo de projetos extensos.

### 🎯 11. Motor de Query Estruturada & Suporte Multi-Projeto
- **Query Graph**: Consultas declarativas avançadas direto no terminal:
  ```bash
  query kind:class layer:domain coupling:>2
  query name:User file:auth.ts
  ```
- **Workspaces Multi-Projeto**: Agregação unificada de monorepos com resolução e isolamento de dependências.
...
*(Qualquer outro comando não reservado, como `git`, `npm`, `cargo`, `claude`, é enviado diretamente ao shell PTY)*
...
│   │   ├── agent-protocol.ts  # Exportação de contexto para LLMs
...
│       ├── components/        # Canvas, Explorer, Inspector, Terminal, etc.
...
O projeto possui uma suíte rigorosa de testes unitários e de integração:
...
# Executar testes unitários do backend Rust (8 testes Cargo)
...
```json
{
  "layerPaths": {
    ...
    "application": ["usecases", "services", "controllers"],
    "infra": ["database", "repositories", "clients", "http"],
...
- [x] **M5.1 — UX Refinada & Nível 5**: Suporte a funções aninhadas, arestas limpas e integração com Git.
...
- [x] **M10 — Protocolo de Agente**: Contexto de símbolos e chamadas pronto para LLMs com notificação de atenção de IA.
